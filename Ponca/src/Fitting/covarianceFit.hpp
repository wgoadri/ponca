...
void CovarianceFitBase<DataPoint, _WFunctor, T>::init(
...
template <class DataPoint, class _WFunctor, typename T>
bool CovarianceFitBase<DataPoint, _WFunctor, T>::addLocalNeighbor(
...
template <class DataPoint, class _WFunctor, typename T>
FIT_RESULT CovarianceFitBase<DataPoint, _WFunctor, T>::finalize()
{
...
#endif
  Base::m_eCurrentState =
      (m_solver.info() == Eigen::Success ? STABLE : UNDEFINED);
...
void CovarianceFitDer<DataPoint, _WFunctor, DiffType, T>::init(
    const VectorType &_evalPos)
{
  Base::init(_evalPos);
...
template <class DataPoint, class _WFunctor, int DiffType, typename T>
bool CovarianceFitDer<DataPoint, _WFunctor, DiffType, T>::addLocalNeighbor(
    Scalar w, const VectorType &localQ, const DataPoint &attributes,
    ScalarArray &dw)
{
...
template <class DataPoint, class _WFunctor, int DiffType, typename T>
FIT_RESULT CovarianceFitDer<DataPoint, _WFunctor, DiffType, T>::finalize()
{
  ...
  if (this->isReady())
  {
    ...
    for (int k = 0; k < Base::NbDerivatives; ++k)
    {
      ...
      m_dCov[k] = m_dCov[k] - cog * Base::m_dSumP.col(k).transpose() -
