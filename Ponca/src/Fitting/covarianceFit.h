...
/*!
   \brief Procedure that compute and decompose the covariance matrix of the
   neighbors positions in \f$3d\f$.

   This process is commonly used for plane fitting and local variance analysis.
   It is often called Principal Component Analysis (PCA) of the neighborhood,
   and used in Geometry Processing and Computer Vision.

   \inherit Concept::FittingProcedureConcept
   \see CovariancePlaneFit which use a similar approach for Plane estimation

   ### Computation details
   Standard PCA algorithm involves a two-steps process where the barycenter
   \f$\mathbf{b}\f$ is first computed, and then the covariance matrix
   \f$\text{C}\f$ (in the following, the weights are ignored for clarity but
   without loss of generality): \f{align}
   \mathbf{b} &= \frac{1}{n}\sum_i\mathbf{p}_i \\
   \text{C}   &=
   \frac{1}{n}\sum_i(\mathbf{p}_i-\mathbf{b})(\mathbf{p}_i-\mathbf{b})^T \f}
   This class implements a single-pass version, where the first formulation is
   re-expressed as follows: \f{align}
   \text{C} &= \frac{1}{n}\sum_i (\mathbf{p}_i\mathbf{p}_i^T -
   \mathbf{b}\mathbf{p}_i^T - \mathbf{p}_i\mathbf{b}^T + \mathbf{b}\mathbf{b}^T)
   \\
            &= \frac{1}{n}\sum_i (\mathbf{p}_i\mathbf{p}_i^T) -
   \frac{1}{n}\sum_i(\mathbf{b}\mathbf{p}_i^T)  -
   \frac{1}{n}\sum_i(\mathbf{p}_i\mathbf{b}^T)  +  \frac{1}{n}\sum_i
   (\mathbf{b}\mathbf{b}^T) \\
            &= \frac{1}{n}\sum_i (\mathbf{p}_i\mathbf{p}_i^T) -
   \mathbf{b}\frac{1}{n}\sum_i(\mathbf{p}_i^T) -
   \frac{1}{n}\sum_i(\mathbf{p}_i)\mathbf{b}^T  + \frac{1}{n}\sum_i(1)
   \mathbf{b}\mathbf{b}^T \\
            &= \frac{1}{n}\sum_i (\mathbf{p}_i\mathbf{p}_i^T) -
   \mathbf{b}\mathbf{b}^T - \mathbf{b}\mathbf{b}^T + \mathbf{b}\mathbf{b}^T \f}
   Leading to a single pass where \f$\text{C}\f$ is express by substracting two
   terms that can be computed independently in one run: \f[ \text{C} =
   \frac{1}{n}\sum_i (\mathbf{p}_i\mathbf{p}_i^T) - \mathbf{b}\mathbf{b}^T \f]
...
public:
  ...
  /*! \brief Implements \cite Pauly:2002:PSSimplification surface variation.
      It computes the ratio \f$ d \frac{\lambda_0}{\sum_i \lambda_i} \f$ with \c
     d the dimension of the ambient space. \return 0 for invalid fits
  */
  ...
  */
  ...
  /*! \brief Implements the eigenentropy.
      ...
      \f[ - \lambda_0 * \ln{\lambda_0} - \lambda_1 * \ln{\lambda_1} -
     \lambda_2 * \ln{\lambda_2} \f]
  */
